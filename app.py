...
async def list_library():
    conn = get_db()
    books = conn.execute('''
        SELECT b.id, b.title, b.author, b.source, b.drive_id, p.cap_idx, p.frag_idx
        FROM books b LEFT JOIN progress p ON b.id = p.book_id
    ...
    for b in books:
        result.append({
            ...
            "source": b["source"],
            "drive_id": b["drive_id"],
            "cap_idx": b["cap_idx"] or 0,
            "frag_idx": b["frag_idx"] or 0
        })
...
async def get_cover(book_id: str):
    ...
    if row and row["cover"]:
        return StreamingResponse(io.BytesIO(row["cover"]), media_type="image/jpeg")
    # Return 404 so frontend can show a placeholder
    raise HTTPException(status_code=404, detail="No cover")

@app.post("/api/library/add")
async def add_book(file: UploadFile = File(...)):
    if not file.filename.lower().endswith(".epub"):
        raise HTTPException(400, "Solo .epub")
    ...
    try:
        ...
        os.remove(epub_path)
        raise HTTPException(422, f"Error parseando EPUB: {e}")

    conn = get_db()
    conn.execute('''
        INSERT INTO books (id, filename, title, author, cover, epub_path, source, added_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    ''', (book_id, file.filename, title, author, cover, epub_path, "local", datetime.now().isoformat()))
    conn.commit()
    conn.close()
